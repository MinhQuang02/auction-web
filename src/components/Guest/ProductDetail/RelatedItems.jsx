...
const RelatedItems = () => {
    ...
    return (
        <section id="related-items" className="container mx-auto px-5 lg:px-12 py-10">
            {/* Header Section */}
            <div className="flex justify-between items-end mb-10">
                <div>
                    ...
                    <h2 className="text-3xl md:text-4xl font-semibold tracking-wide">Related Items</h2>
                </div>
                ...
                {products.map((product) => (
                    <div key={product.id} className="flex-none w-[270px] snap-center">
                        ...
                        <div className="relative bg-secondary rounded-md shadow-md h-[250px] flex justify-center items-center overflow-hidden mb-4 group">
                            <img src={product.image} alt={product.title} className="max-h-[180px] object-contain p-4" />
                            ...
                            <a href="#" className="absolute bottom-0 w-full bg-black text-white text-center py-2 font-medium opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                Bid Now
                            </a>
                        </div>

                        {/* Product Info */}
                        <h3 className="font-medium text-base mb-2 truncate" title={product.title}>{product.title}</h3>
                        <div className="flex gap-3 items-center mb-2">
                            <span className="text-primary font-medium">{product.currentPrice}</span>
                            <span className="text-gray-500 opacity-50 font-medium">by {product.bidder}</span>
                        </div>
