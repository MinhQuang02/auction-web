        ...
        {[...Array(5)].map((_, index) => (
          <svg
            ...
            className={`w-4 h-4 fill-current ${
              index < filledCount ? "" : "text-gray-300"
            }`}
            viewBox="0 0 20 20"
          >
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1.6fr_1fr] gap-8 h-auto xl:h-[480px]">
        ...
        <div className="flex flex-col-reverse md:flex-row gap-4 h-full w-full">
          ...
          <div className="flex md:flex-col gap-4 justify-between w-full md:w-[110px] flex-none h-auto md:h-full overflow-x-auto md:overflow-hidden">
            {[0, 1, 2, 3].map((item, index) => {
              ...
              return (
                <div
                  key={index}
                  className="bg-[#F2F2F2] rounded-xl shadow-sm h-[80px] w-[80px] md:h-[110px] md:w-full flex-none flex items-center justify-center cursor-pointer border border-transparent hover:border-black transition p-2"
                >
                  <img
                    src={mainProductImg}
                    className={`w-14 h-auto object-contain ${transformClass}`}
                    alt={`Thumb ${index + 1}`}
                  />
                </div>
              );
          ...
          <div className="bg-[#F2F2F2] rounded-2xl shadow-lg flex-grow flex items-center justify-center p-8 w-full h-[300px] md:h-full">
            <img
              src={mainProductImg}
              className="w-auto h-auto max-h-[240px] max-w-full object-contain drop-shadow-2xl"
              alt="Main Product"
            />
          </div>
        </div>
        ...
        <div className="flex flex-col h-full justify-between py-1">
          <div>
            ...
            <div className="text-sm text-gray-700 space-y-1 mb-3">
              ...
              <p>
                • Auction End Time:{" "}
                <span className="font-medium">19/07/2025</span>
              </p>
            </div>
