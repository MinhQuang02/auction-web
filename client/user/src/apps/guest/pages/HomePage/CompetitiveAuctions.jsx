...
const CompetitiveAuctions = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const res = await fetch("http://localhost:8000/api/products/competitive?limit=5");
        if (res.ok) {
          const data = await res.json();
          setProducts(data);
        }
      } catch (err) {
        console.error("Failed to fetch competitive products", err);
      }
    };
    fetchProducts();
  }, []);
      ...
      <div className="container mx-auto px-5 lg:px-12 relative">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-8 relative z-10">
          {products.map((product, index) => {
            ...
            // Calculation for "Competitive" context? Just show price for now since it's "High Price/Value" logic.
