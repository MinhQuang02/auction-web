...
class EmailService {
    ...
    async sendOtp(email, otp, type = 'verification') {
        ...
        await sendEmail(email, `${type === 'reset' ? 'Reset Password' : 'Verify Email'} - bid.dify`, html);
    }
    ...
    // B. Ask Seller
    async sendQuestionNotification(sellerEmail, sellerName, askerName, productName, productId, questionText) {
        const productUrl = `${process.env.FRONTEND_URL}/product/${productId}`;
        const html = getQuestionTemplate(sellerName, askerName, productName, productUrl, questionText);
        await sendEmail(sellerEmail, `New Question: ${productName}`, html);
    }

    // C. Transaction
    async sendTransactionEmails(buyer, seller, product, transaction, shippingData) {
        // Buyer Receipt
        const buyerHtml = getBuyerReceiptTemplate(buyer.full_name, transaction, product);
        sendEmail(buyer.email, `Receipt for Order #${transaction.transaction_id}`, buyerHtml);

        // Seller Notification
        const sellerHtml = getSellerOrderTemplate(seller.full_name, transaction, product, shippingData);
    ...
    async sendOutbidNotifications(bidders, productName, productId, newPrice) {
        ...
        const promises = bidders.map(bidder => {
            const html = getOutbidTemplate(bidder.full_name, productName, productUrl, newPrice);
