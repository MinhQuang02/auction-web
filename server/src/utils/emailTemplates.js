...
<head>
  ...
  <title>${title}</title>
  <style>
    body { margin: 0; padding: 0; font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; background-color: ${
      BRAND_COLORS.background
    }; color: ${BRAND_COLORS.text}; }
    .container { max-width: 600px; margin: 0 auto; background-color: ${
      BRAND_COLORS.white
    }; border-radius: 12px; overflow: hidden; margin-top: 30px; box-shadow: 0 8px 16px rgba(60, 45, 30, 0.15); }
    .header { background-color: ${BRAND_COLORS.primary}; color: ${
    BRAND_COLORS.white
  }; padding: 30px; text-align: center; }
    .header h1 { margin: 0; font-size: 28px; letter-spacing: 2px; font-weight: 800; }
    .content { padding: 40px 30px; line-height: 1.6; background-color: ${
      BRAND_COLORS.white
    }; }
    .footer { background-color: #5E4B35; padding: 30px; text-align: center; font-size: 13px; color: #E0E0E0; }
    .footer p { margin: 5px 0; }
    .btn { display: inline-block; padding: 14px 28px; background-color: ${
      BRAND_COLORS.primary
    }; color: #ffffff !important; text-decoration: none; border-radius: 6px; font-weight: bold; margin-top: 20px; text-transform: uppercase; letter-spacing: 1px; }
    .btn:hover { background-color: #96836C; }
    .info-box { background-color: #FAF8F5; border: 1px solid ${
      BRAND_COLORS.border
    ...
    .info-value { font-weight: bold; font-size: 14px; color: ${
      BRAND_COLORS.text
    }; }
  </style>
</head>
  ...
  const content = `
    ...
    <div style="text-align: center; margin: 30px 0;">
      <span style="font-size: 32px; font-weight: bold; letter-spacing: 5px; color: ${
        BRAND_COLORS.text
      }; background: #f0f0f0; padding: 10px 20px; border-radius: 8px; border: 1px dashed #ccc;">${otp}</span>
    </div>
...
export const getBuyerReceiptTemplate = (userName, transaction, product) => {
  const content = `
    ...
    <div style="text-align: center;">
      ...
      }/my-purchases" class="btn">View Order</a>
    </div>
  `;
  return getBaseTemplate(content, "Payment Receipt");
};

export const getSellerOrderTemplate = (
  sellerName,
  transaction,
  product,
  shippingData
  ...
  const address =
    typeof shippingData === "string"
      ? JSON.parse(shippingData || "{}")
      : shippingData;
  ...
  const content = `
    ...
    <p>Your item <strong>${
      product.name
    }</strong> has been sold and paid for.</p>
    ...
    <div style="text-align: center;">
      <a href="${
        process.env.FRONTEND_URL || "#"
      }/my-products" class="btn">Manage Order</a>
    </div>
  `;
  return getBaseTemplate(content, "You made a sale!");
};

export const getOutbidTemplate = (
  bidderName,
  productName,
  productUrl,
  newPrice
  ...
  const content = `
    ...
    <div style="text-align: center; margin: 20px 0;">
      ...
      <p style="font-size: 24px; font-weight: bold; color: #AE9B84;">$${Number(
        newPrice
      ).toLocaleString()}</p>
    </div>
...
export const getNewPasswordTemplate = (fullName, newPassword) => {
  const content = `
    <h2>Password Reset by Admin</h2>
    <p>Hi <strong>${fullName}</strong>,</p>
    <p>An administrator has reset your account password. Your new password is:</p>
    <div style="text-align: center; margin: 30px 0;">
      <span style="font-size: 24px; font-weight: bold; letter-spacing: 2px; color: ${BRAND_COLORS.text}; background: #f0f0f0; padding: 10px 20px; border-radius: 8px; border: 1px dashed #ccc;">
        ${newPassword}
      </span>
    </div>
    <p>Please log in and change your password immediately to something secure.</p>
  `;
